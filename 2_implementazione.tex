...
\begin{algorithm}
 \begin{algorithmic}[1]
	...
	\State \Return END\_OK
      \Else 
	\ForAll {($node$ \textbf{in} $nodes$ \textbf{with} [$seen\_news$ = \textbf{false}])}
	  \If {(\textbf{any} $node_{neighbors}$ \textbf{with} [$shared\_news$ = \textbf{true}])}\\
	    ...
	    \If { $p\_news \geq node.chance\_of\_abstention$ }
	  ...
	  \EndIf
	\EndFor
	\If {(\textbf{no} $nodes$ \textbf{with} [$tmp\_seen\_news$ = \textbf{True}])}
	  \State \Return END\_KO
	\EndIf
	
	\ForAll {($node$ \textbf{in} $nodes$ \textbf{with} [$tmp\_seen\_news$ = \textbf{true}])}
	  \State $node_{seen\_news} \gets \textbf{true}$
	  \State $node_{tmp\_seen\_news} \gets \textbf{false}$
	\EndFor
	
	\ForAll {($node$ \textbf{in} $nodes$ \textbf{with} [$tmp\_shared\_news$ = \textbf{true}])}
...
Per questa analisi è stato sviluppato un secondo Algoritmo, di tre cicli annidati che permette di considerare

...
In tutti gli altri utenti del grafo verrà, invece, impostato lo stato ``Ignorant'' e sarà calcolato un nuovo valore di 
...
I pulsanti presenti in figura~\ref{img:gui_first_second_test} danno la possibilità di far partire la simulazione per il primo e secondo obiettivo.
Più precisamente per la prima simulazione viene automaticamente impostato il tipo di Social Netwok a ``custom'' per poter 
personalizzare la percentuale di utenti nei vari gruppi, impostando il 100\% nel primo gruppo e 0\% negli altri gruppi. 
A questo punto il test eseguirà la procedura di propagazione della notizia facendo variare la probabilità della notizia da 0 a 1 con step di 0.05 per il primo gruppo.
